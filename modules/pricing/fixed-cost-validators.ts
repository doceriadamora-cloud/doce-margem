...
export function validateFixedCost(cost: FixedCost): ValidationError[] {
  ...
  // NaN/Infinity antes das comparações: NaN faz `<` devolver false e passaria.
  if (!Number.isFinite(cost.monthlyValue)) {
    errors.push({
      field: "monthlyValue",
      code: "INVALID_NUMBER",
      message: `Informe um número válido para o valor mensal do custo fixo "${cost.name}".`,
    });
  } else if (cost.monthlyValue < 0) {
    errors.push({
      field: "monthlyValue",
      code: "NEGATIVE",
...
export function validateFixedCostCalculation(
  ...
  if (!Number.isFinite(input.estimatedMonthlyRevenue)) {
    errors.push({
      field: "estimatedMonthlyRevenue",
      code: "INVALID_NUMBER",
      message: "Informe um número válido para o faturamento estimado.",
    });
  } else if (input.estimatedMonthlyRevenue <= 0) {
    errors.push({
      field: "estimatedMonthlyRevenue",
      code: "NON_POSITIVE",
  ...
  if (input.estimatedMonthlyUnits !== undefined) {
    if (!Number.isFinite(input.estimatedMonthlyUnits)) {
      errors.push({
        field: "estimatedMonthlyUnits",
        code: "INVALID_NUMBER",
        message: "Informe um número válido para o volume estimado.",
      });
    } else if (input.estimatedMonthlyUnits <= 0) {
      errors.push({
        field: "estimatedMonthlyUnits",
        code: "NON_POSITIVE",
  ...
  if (input.includeChannelMonthlyFees && input.channels) {
    input.channels.forEach((channel, index) => {
      if (!Number.isFinite(channel.monthlyFee)) {
        errors.push({
          field: `channels[${index}].monthlyFee`,
          code: "INVALID_NUMBER",
          message: `Informe um número válido para a mensalidade do canal "${channel.name}".`,
        });
      } else if (channel.monthlyFee < 0) {
        errors.push({
          field: `channels[${index}].monthlyFee`,
          code: "NEGATIVE",
